...
class PyTorchGTSRB(VisionDataset):
    ...
    def __init__(
        ...
        if corruption is not None:
            print(">>>> GTSRB corruption style:" + corruption)
            self._target_folder = (
                self._base_folder
                / "GTSRB"
                / (
                    ...
                    else "Final_Test_Corruption/severity_2/" + corruption + "/Images"
                )
            )
        else:
            self._target_folder = (
...
class GTSRB:
    def __init__(
        ...
        logger.info(f"loading GTSRB dataset with corruption {corruption}")
