...
class ConcreteMask(nn.Module):
    ...
    def _apply_mask(self, concrete_masks, state_dict: StateDict):
        ...
        for k, v in state_dict.items():
            ...
            print(concrete_mask.mean().item())
        return new_state_dict

    def apply_mask(
        self, state_dicts: List[StateDict], concrete_masks: Optional[StateDict] = None
    ):
        """
    ...
    def state_dict(self, keep_vars: bool = False):
        if keep_vars:
            return {k: v for k, v in self.masks.items()}
        else:
            return {k: v.detach() for k, v in self.masks.items()}
    ...
    def to(self, device):
        for k in self.masks.keys():
            self.masks[k].data = self.masks[k].to(device)
        return self
