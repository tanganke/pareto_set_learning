...
def draw_connectivity_graph(graph, value_key, weight_key, pos=None):
    ...
    nx.draw_networkx_nodes(
        graph, pos, nodelist, node_color=node_colour, cmap=cmap, vmin=vmin, vmax=vmax
    )
...
def plot_dense(dense_data, pivot_distances, normed_length=False):
    ...
    plt.plot(
        x.numpy()[pivot_slice], dense_data.numpy()[pivot_slice], "d", c=line.get_color()
    )

    plt.xlabel("Position on path")
    plt.xticks(
        [0] + list(pivot_distances.cumsum(0)),
        ["$ \\theta_1 $"] + ([""] * (pivot_distances.shape[0] - 1)) + ["$ \\theta_2 $"],
    )
...
def x_for_dense(data, distances, normed_length=False):
    ...
    for j in range(sub_image_count + 1):
        ...
        sub_normed[j : -1 : sub_image_count + 1] = (1 - alpha) * normed[
            0:-1
        ] + alpha * normed[1:]
...
class Cluster:
    def __init__(
        self,
        cluster_a: Union["Cluster", Leaf],
        cluster_b: Union["Cluster", Leaf],
        value,
    ):
...
def draw_disconnectivity_graph(graph, value_key, weight_key):
    ...
    clusters: List[Union[Leaf, Cluster]] = [
        Leaf(m, mst.nodes[m][value_key]) for m in mst
    ]
