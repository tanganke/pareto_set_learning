...
from torch.nn import AvgPool2d, BatchNorm2d, Conv2d, Linear, Module, ReLU, Sequential
...
class BasicBlock(Module):
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, non_linearity=ReLU()
    ):
...
class ResNet(Module):
    ...
    def _make_layer(self, planes, blocks, stride, non_linearity):
        ...
        layers = [
            BasicBlock(
                self.inplanes, planes, stride, downsample, non_linearity=non_linearity
            )
        ]
        self.inplanes = planes
        for i in range(1, blocks):
            layers.append(
                BasicBlock(self.inplanes, planes, non_linearity=non_linearity)
            )
