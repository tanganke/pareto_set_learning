...
from torch_autoneb.models.simple import CurvyValley, Eggcarton, Flat, Linear
...
def param_init(mod: Module):
    if isinstance(mod, nn.Linear):
        n = mod.in_features
        mod.weight.data.normal_(0, 1.0 / sqrt(n))
...
class ModelWrapper(ModelInterface):
    ...
    def __init__(
        ...
        self.coords = (
            torch.empty(self.number_of_dimensions, dtype=torch.float32)
            .to(device)
            .zero_()
        )
    ...
    def get_coords(
        self, target: Tensor = None, copy: bool = True, update_cache: bool = True
    ) -> Tensor:
        """
...
def ensure_data_loader(
    data_provider: [Dataset, DataLoader], batch_size, **kwargs
) -> DataLoader:
