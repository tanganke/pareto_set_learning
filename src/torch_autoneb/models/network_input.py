...
class NetworkInputModel(Module):
    ...
    def __init__(
        ...
        input_size: tuple,
        label: int,
        clamp_data: bool = True,
        stop_in_target_region: bool = False,
    ):
        super().__init__()
    ...
    def initialise_randomly(self):
        raise NotImplementedError(
            "You need to manually prepare a set of minima, have a look at torch_autoneb.models.network_input.NetworkInputModel.generate_dataset()"
        )

    def generate_dataset(
        self, dataset, number_of_classes, valid_set_size=50, adversarial_set_size=50
    ):
        final_set = []
        adversarial_set = []
        ...
        for i in pbar(
            range(valid_set_size + adversarial_set_size), desc="Generating dataset"
        ):
            ...
            if target == self.label:
                ...
                if len(adversarial_set) < adversarial_set_size:
                    ...
                    user_data = self.analyse()
                    user_data.update({"adversarial": True, "original_class": target})
    ...
    def forward(self, **kwargs):
        ...
        if self.training and self.stop_in_target_region:
            ...
            if is_correct_label:
                ...
                happy_with_input = (
                    probs[0, self.label] - probs[0, hard_pred[0, 1]] > 0.05
                )
            else:
    ...
    def analyse(self):
        ...
        analysis = {
            ...
            "softmax_correct_inverse": 1
            - softmax(soft_pred, 1).data[:, self.label].cpu()[0],
            "cross_entropy": nll_loss(
                log_softmax(soft_pred, 1).cpu(), Variable(LongTensor([self.label]))
            ).item(),
