...
def equal(previous_cycle_data, count):
    ...
    return fill_chain(
        path_coords,
        [torch.linspace(0, 1, count + 2)[1:-1]] * (path_coords.shape[0] - 1),
        weights,
    )
...
def highest(previous_cycle_data: dict, count: int, key: str, threshold=0.1):
    ...
    assert (dense_data.shape[0] - 1) % (
        path_coords.shape[0] - 1
    ) == 0, f"Bad shape of dense data {dense_data.shape}."
    ...
    for i in range(path_coords.shape[0] - 1):
        ...
        for j, alpha in enumerate(alphas):
            scores[i, j] = dense_data[idx_a + j + 1] - (
                score_a * (1 - alpha) + score_b * alpha
            )
...
def fill_chain(
    existing_chain: Tensor, insert_alphass: list, relative_lengths: Tensor = None
):
    """
    ...
    assert (
        len(insert_alphass) == existing_count - 1
    ), "For each connection in line, a number of new items must be specified!"
