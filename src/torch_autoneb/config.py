...
class OptimConfig(BaseConfig):
    ...
    def from_dict(config_dict: dict):
        ...
        (
            config_dict["algorithm_type"],
            config_dict["algorithm_args"],
        ) = replace_instanciation(config_dict["algorithm"], optim)
        del config_dict["algorithm"]
        if "scheduler" in config_dict:
            (
                config_dict["scheduler_type"],
                config_dict["scheduler_args"],
            ) = replace_instanciation(config_dict["scheduler"], lr_scheduler)
...
class NEBConfig(BaseConfig):
    ...
    def from_dict(config_dict: dict):
        config_dict = deepcopy(config_dict)
        (
            config_dict["insert_method"],
            config_dict["insert_args"],
        ) = replace_instanciation(config_dict["insert"], torch_autoneb.fill)
...
class LandscapeExplorationConfig(BaseConfig):
    def __init__(
        ...
        value_key: str,
        weight_key: str,
        suggest_methods: list,
        suggest_args: list,
        auto_neb_config: AutoNEBConfig,
    ):
    ...
    def from_dict(config_dict: dict):
        ...
        config_dict["suggest_methods"], config_dict["suggest_args"] = zip(
            *[
                replace_instanciation(engine, suggest)
                for engine in config_dict["suggest"]
            ]
        )
