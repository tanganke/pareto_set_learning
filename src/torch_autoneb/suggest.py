...
from networkx import Graph, MultiGraph, connected_components, minimum_spanning_tree
        ...
        for m2 in graph[m1]:
            ...
            if logger is not None:
                ...
                if cycles != intermediate:
                    logger.warning(
                        f"Minima {m1} and {m2} are missing intermediate cycles {intermediate.difference(cycles)}."
                    )
...
def mst(graph: MultiGraph, config):
    ...
    highest_edges = sorted(
        mst.edges, key=lambda e: simple_graph[e[0]][e[1]][weight_key], reverse=True
    )
    ...
    for highest_edge in highest_edges:
        ...
        for source_minimum in sorted(
            cluster1, key=lambda x: simple_graph.nodes[x][value_key]
        ):
            for target_minimum in sorted(
                cluster2, key=lambda x: simple_graph.nodes[x][value_key]
            ):
                if (
                    source_minimum,
                    target_minimum,
                ) not in simple_graph.edges and source_minimum is not target_minimum:
    ...
    for source_minimum in sorted(graph, key=lambda x: graph.nodes[x][value_key]):
        for target_minimum in sorted(graph, key=lambda x: graph.nodes[x][value_key]):
            if (
                source_minimum,
                target_minimum,
            ) not in graph.edges and source_minimum is not target_minimum:
                return source_minimum, target_minimum
