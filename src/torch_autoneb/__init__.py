...
import torch
from networkx import Graph, MultiGraph, minimum_spanning_tree
...
def find_minimum(model: models.ModelWrapper, optim_config: config.OptimConfig) -> dict:
    optimiser = optim_config.algorithm_type(
        model.parameters(), **optim_config.algorithm_args
    )  # type: optim.Optimizer

    # Scheduler
    if optim_config.scheduler_type is not None:
        scheduler = optim_config.scheduler_type(
            optimiser, **optim_config.scheduler_args
        )
    else:
        scheduler = None
...
def neb(
    previous_cycle_data, model: models.ModelWrapper, neb_config: config.NEBConfig
) -> dict:
    ...
    if "weight_decay" in optimiser.defaults:
        assert (
            optimiser.defaults["weight_decay"] == 0
        ), "NEB is not compatible with weight decay on the optimiser. Set weight decay on NEB instead."
    ...
    saddle_analysis = {
        key: value for key, value in analysis.items() if "saddle_" in key
    }
...
def auto_neb(
    ...
    if m2 in graph[m1]:
        ...
        connection_data = {
            "path_coords": torch.cat(
                [graph.nodes[m]["coords"].view(1, -1) for m in (m1, m2)]
            ),
    ...
    for engine, engines_args in zip(config.suggest_methods, engines_args):
        ...
        if m1 is None or m2 is None:
            ...
            if logger is not None:
                logger.info(
                    "Connecting %d <-> %d based on %s." % (m1, m2, engine.__name__)
                )
            assert m1 is not m2
            return m1, m2
    return None, None
        ...
        with helper.pbar(desc="Landscape Exploration") as bar:
            while True:
                ...
                if m1 is None or m2 is None:
                    break
                auto_neb(
                    m1, m2, graph, model, lex_config.auto_neb_config, callback=callback
                )
                bar.update()
                ...
                in_mst_str = (
                    "included"
                    if minimum_spanning_tree(simple_graph, weight_key)
                    else "not included"
                )
                logger.info(
...
def to_simple_graph(
    graph: MultiGraph, weight_key: str, cycle_count: int = None
) -> Graph:
    """
    ...
    for m1 in graph:
        for m2 in graph[m1]:
            ...
            best_edge_key = min(
                graph[m1][m2], key=lambda key: graph[m1][m2][key][weight_key]
            )
