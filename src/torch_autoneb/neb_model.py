...
class NEB(models.ModelInterface):
    ...
    def apply(self, gradient=False, **kwargs):
        ...
        assert (
            self.target_distances is not None or not gradient
        ), "Cannot compute gradient if target distances are unavailable"
        if gradient and self.spring_constant == float("inf"):
            self.path_coords[:] = distribute_by_weights(
                self.path_coords,
                self.path_coords.shape[0],
        ...
        if gradient:
            ...
            for i in range(1, npivots - 1):
                ...
                td_prev, td_next = (
                    self.target_distances[i - 1].item(),
                    self.target_distances[i].item(),
                )
                l_prev, loss, l_next = (
                    losses[i - 1].item(),
                    losses[i].item(),
                ...
                self.path_coords.grad[i] -= (
                    self.path_coords.grad[i].dot(tangent) * tangent
                )
    ...
    def iterate_densely(self, sub_pivot_count=9):
        ...
        for i in helpers.pbar(range(dense_pivot_count), "Saddle analysis"):
            ...
            if sub_pivot == 0:
                ...
                coords = (
                    self.path_coords[base_pivot] * (1 - alpha)
                    + self.path_coords[base_pivot + 1] * alpha
                )
    ...
    def analyse(self, sub_pivot_count=9):
        ...
        analysis["lengths"] = (self.path_coords[1:] - self.path_coords[:-1]).norm(
            2, 1
        ) / end_to_end_distance
...
def distribute_by_weights(
    path: Tensor,
    nimages: int,
    path_target: Tensor = None,
    weights: Tensor = None,
    climbing_pivots: list = None,
):
    """
    ...
    if path_target is None:
        ...
        assert (
            path_target is not path
        ), "Source must be unequal to target for redistribution"
    ...
    if climbing_pivots is not None:
        assert (
            path.shape[0] == nimages
        ), "Cannot change number of items when reinterpolating with respect to climbing images."
        assert len(climbing_pivots) == nimages
        assert all(
            isinstance(b, bool) for b in climbing_pivots
        ), "Image must be climbing or not."
        start = 0
    ...
    target_positions = (weights / weights.sum()).cumsum(
        0
    ) * current_distances.sum()  # Target positions of elements (spaced by weights)

    # Put each new item spaced by weights (measured along line) on the line
    last_idx = 0  # Index of previous pivot
    pos_prev = 0.0  # Position of previous pivot on chain
