...
from src.min_norm_solvers.min_norm_solvers import MinNormSolver


def compute_mgda_loss(losses: Tuple[Tensor], params: Tuple[nn.Parameter]):
    grads = []
    for loss in losses:
        ...
        grad = tuple(filter(lambda x: x is not None, grad))
        grad = tuple(map(lambda x: x.flatten(), grad))
        grad = torch.cat(grad)
        grads.append(grad)
    sol, min_norm = MinNormSolver.find_min_norm_element(grads)

    loss = 0
    for w, l in zip(sol, losses):
        loss += w * l
    return loss
...
class Program:
    ...
    def train(self):
        ...
        for step_idx in tqdm(
            ...
            loss = compute_mgda_loss(
                losses, tuple(filter(lambda p: p.requires_grad, model.parameters()))
            )
